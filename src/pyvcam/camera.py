...
class Camera:
            ...
            try:
                if isinstance(keyOrValue, str):
                    ...
                    return [
                        ...
                        if keyOrValue == item_value
                    ][0]
            except KeyError:
                raise ValueError(
                    "Invalid key: {0} for {1} - Available keys are: {2}".format(
                        keyOrValue, self.name, list(self.keys())
                    )
                )
            except IndexError:
                raise ValueError(
                    "Invalid value: {0} for {1} - Available values are: {2}".format(
                        keyOrValue, self.name, list(self.values())
                    )
                )
    ...
    def open(self) -> None:
        ...
        self.__defaultRoi = Camera.RegionOfInterest(
            0, self.sensor_size[0] - 1, 1, 0, self.sensor_size[1] - 1, 1
        )
        self.reset_rois()
        ...
        for port_name, port_value in self.read_enum(const.PARAM_READOUT_PORT).items():
            ...
            for speed_index in range(num_speeds):
                ...
                gain_increment = self.get_param(
                    const.PARAM_GAIN_INDEX, const.ATTR_INCREMENT
                )

                numGains = int((gain_max - gain_min) / gain_increment + 1)
                gains = [(gain_min + i * gain_increment) for i in range(numGains)]

                self.__port_speed_gain_table[port_name].update(
                    {
                        speed_name: {
                ...
                for gain_index in gains:
                    ...
                    try:
                        gain_name = self.get_param(
                            const.PARAM_GAIN_NAME, const.ATTR_CURRENT
                        )
                    except Exception:
        ...
        try:
            ...
            for featureIndex in range(featureCount):
                ...
                paramCount = self.get_param(
                    const.PARAM_PP_PARAM_INDEX, const.ATTR_COUNT
                )
    ...
    def check_frame_status(self) -> str:
        ...
        Returns:
            ...
            'EXPOSURE_IN_PROGRESS' - The data collection routines are @b active.
                                     They are waiting for data to arrive, but none
                                     has arrived yet.
        ...
        # TODO:  pvcam currently returns FRAME_AVAILABLE/READOUT_COMPLETE after a
        #        sequence is finished. Until this behavior is resolved,
    ...
    def set_roi(self, s1: int, p1: int, w: int, h: int) -> None:
        ...
        inBounds = (
            0 <= s1 <= self.sensor_size[0]
            and 0 <= s2 <= self.sensor_size[0]
            and 0 <= p1 <= self.sensor_size[1]
            and 0 <= p2 <= self.sensor_size[1]
        )
    ...
    def poll_frame(
        ...
        Parameter:
            oldestFrame (bool): Selects whether to return the oldest or newest frame.
                                Only the oldest frame will be popped off the underlying
                                queue of frames. (optional).
            copyData (bool): Selects whether to return a copy of the numpy frame which
                             points to a new buffer, or the original numpy frame which
                             points to the buffer used directly by PVCAM. Disabling
                             this copy is not recommended for most situations. Refer to
                             PyVCAM Wrapper.md for more details. (optional).

            None
        Returns:
            A dictionary with the frame containing available meta data and 2D np.array
        ...
        for i in range(num_frames):
            ...
            try:
                ...
                stack[i] = self.get_frame(
                    exp_time=self.vtm_exp_time, timeout_ms=timeout_ms
                )
            except Exception:
            ...
            if param_name in self.__post_processing_table[feature_name].keys():
                ...
                if pp_param["param_min"] <= value <= pp_param["param_max"]:
                    ...
                    raise AttributeError(
                        "Could not set post processing param. Value "
                        + str(value)
                        + " out of range ("
                        + str(pp_param["param_min"])
                        + ", "
                        + str(pp_param["param_max"])
                        + ")"
                    )
            else:
                raise AttributeError(
                    "Could not set post processing param. param_name not found"
                )
        else:
            raise AttributeError(
    ...
    def readout_port(self, value: int) -> None:
        ...
        if value >= num_ports:
            raise ValueError(
                "{} only supports " "{} readout ports.".format(self, num_ports)
            )
    ...
    def speed_table_index(self, value: int) -> None:
        ...
        if value >= num_entries:
            raise ValueError(
                "{} only supports " "{} speed entries".format(self, num_entries)
            )
    ...
    def gain(self, value: int) -> None:
        ...
        if not (min_gain <= value <= max_gain):
            raise ValueError(
                "Invalid value: {} - {} only supports gain "
    ...
    def bin_x(self, value: int) -> None:
        ...
        raise ValueError(
            "{} only supports {} binnings".format(
                self, self.read_enum(const.PARAM_BINNING_SER).items()
            )
        )
    ...
    def bin_y(self, value: int) -> None:
        ...
        raise ValueError(
            "{} only supports {} binnings".format(
                self, self.read_enum(const.PARAM_BINNING_PAR).items()
            )
        )
    ...
    def exp_res(self, keyOrValue: Union[int, str]) -> None:
        ...
        assert value in self.__exp_resolutions
    ...
    def exp_time(self, value: int) -> None:
        ...
        if value not in range(min_exp_time, max_exp_time + 1):
            raise ValueError(
                "Invalid value: {} - {} only supports exposure "
                "times between {} and {}".format(
                    value, self, min_exp_time, max_exp_time
                )
            )
    ...
    def exp_mode(self, keyOrValue: Union[int, str]) -> None:
        ...
        # Verify value is in range by attempting to look-up the key
        assert self.__exp_mode in self.__exp_modes
    ...
    def exp_out_mode(self, keyOrValue: Union[int, str]) -> None:
        ...
        # Verify value is in range by attempting to look-up the key
        assert self.__exp_out_mode in self.__exp_out_modes
    ...
    def clear_mode(self, keyOrValue: Union[int, str]) -> None:
        ...
        value = (
            self.__clear_modes[keyOrValue]
            if isinstance(keyOrValue, str)
            else keyOrValue
        )

        # Verify value is in range by attempting to look-up the key
        assert value in self.__clear_modes
    ...
    def temp_setpoint(self, value: int) -> None:
        ...
        try:
            ...
            raise ValueError(
                "Invalid temp {} : Valid temps are in the range {} "
    ...
    def centroids_mode(self, keyOrValue: Union[int, str]) -> None:
        ...
        value = (
            self.__centroids_modes[keyOrValue]
            if isinstance(keyOrValue, str)
            else keyOrValue
        )

        # Verify value is in range by attempting to look-up the key
        assert value in self.__centroids_modes
    ...
    def prog_scan_mode(self, keyOrValue: Union[int, str]) -> None:
        ...
        # provided with an unrecognized key
        value = (
            self.__prog_scan_modes[keyOrValue]
            if isinstance(keyOrValue, str)
            else keyOrValue
        )

        # Verify value is in range by attempting to look-up the key
        assert value in self.__prog_scan_modes
    ...
    def prog_scan_dir(self, keyOrValue: Union[int, str]) -> None:
        ...
        value = (
            self.__prog_scan_dirs[keyOrValue]
            if isinstance(keyOrValue, str)
            else keyOrValue
        )

        # Verify value is in range by attempting to look-up the key
        assert value in self.__prog_scan_dirs
